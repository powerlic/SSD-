...
//从数据中读取变换参数，初始化param_，同时如果设置了均值文件，则利用文件初始化一个均值blob,为后续转换做准备。
...
//将Datum根据转换系数，转换成Dtype类型，如果有镜像，则翻转，如果有均值，则减去均值，同时得到包围框，注意框是float型
template<typename Dtype>
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       Dtype* transformed_data,
                                       NormalizedBBox* crop_bbox,
                                       bool* do_mirror) {
  //由于datum 中data数据类型是byte,所以可以用string直接读取。
...
template<typename Dtype>
//是否用图像格式封装，决定数据是否调用DecodeDatumToCVMat进行转换，否则调用基础Transform函数进行转换
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       Blob<Dtype>* transformed_blob,
                                       NormalizedBBox* crop_bbox,
                                       bool* do_mirror) {
	//如果数据是封装的，则转化成opencv image
...
//利用图像高宽推断维度，注意如果切割了，则需要设置为crop_h,crop_w
#ifdef USE_OPENCV
template<typename Dtype>
